...
def regextract(regex_query, source_column, destination_column, sqlfilter=''):
    ...
    sqlstring = 'select count(*) from Decisions'
    if sqlfilter !='':
        sqlstring=sqlstring+' where {}'.format(sqlfilter)
    ...
    print(sqlfilter, "--", 'Total:', str(total_records) + '.',
          'Blanks:', str(start_amount) + '.', 'Added:', end='')
...
def Branch():
    print("Adding Branch...")
    regextract("AF|ARMY|CG|Marine|NAVY", "local_filepath", "Branch")
...
def DecisionDate():
    print("Adding Decision Year...")
    regextract("CY([0-9]{4})|([0-9]{4})_", "local_filepath", "Year")
